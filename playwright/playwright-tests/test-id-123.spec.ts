...
test('Assing badge first step then navigate to second step', async ({ page }) => {
  ...
  // 2. Verify menu item with label 'Badge' is present in the left menu
  ...
  // 4. Verify sub menu item with label 'Tutti i Badge' is present
  ...
  const assegnaButton = page.getByRole('button', { name: 'Assegna', exact: true });
  await expect(assegnaButton).toBeVisible();

  // 7. Click on 'Assegna' button
  await assegnaButton.click();
  ...
  // 16. Verify the page has a button with text 'Seleziona Anagrafica'
  ...
  // 17. Click on 'Seleziona da Anagrafica' button
  ...
  const firstRow = page.getByRole('row', { name: /giulia verdi giulia.verdi/i });
